...
```{r}
 ...
 geom=c("point","smooth"),method="lm")+
 labs(title =
 "ScatterPlotofRelationshipBetween
 Socialand HappinessScore",
 x="Social",y="HappinessScore")
```
...
qplot(x= Health,y=Happiness_Score,data=data,
 ...
 labs(title =
 "ScatterPlotofRelationshipBetween
 Healthand HappinessScore",
 x="Health",y="HappinessScore")
```
...
ggplot(aes(x = Economy, y = Happiness_Score)) +
  ...
  labs(title = "HeatMap with linearline (Economy and Happiness Score)", 
...
ggplot(aes(x = Social, y = Happiness_Score)) +
  geom_bin2d(bins = 30) +  
  geom_smooth(method = "lm", se = FALSE) + 
  labs(title = "HeatMap with linearline (Social and Happiness Score)", 
...
ggplot(aes(x = Health, y = Happiness_Score)) +
  geom_bin2d(bins = 30) +  
  geom_smooth(method = "lm", se = FALSE) + 
  labs(title = "HeatMap with linearline (Health and Happiness Score)", 
...
ggplot(aes(x = Freedom, y = Happiness_Score)) +
  geom_bin2d(bins = 30) +  
  geom_smooth(method = "lm", se = FALSE) + 
  labs(title = "HeatMap with linearline (Freedom and Happiness Score)", 
...
ggplot(aes(x = Corruption, y = Happiness_Score)) +
  geom_bin2d(bins = 30) +  
  geom_smooth(method = "lm", se = FALSE) + 
  labs(title = "HeatMap (Corruption and Happiness Score)", 
...
colnames(combined_data) <- c("Mean", "Median", "Max", "IQR", "Variables")
```

```{r}
group_data2 <- combined_data %>%
 pivot_longer(cols = Mean:IQR, names_to = "Summary_Variable" , values_to = "Value")
```
...
visualize(null_distribution_correlation) +
  shade_p_value(obs_stat = happiness_obs_stat, direction = "two_sided") +
    labs(title = "Null Distribution of Happiness Score", x = "Correlation", y = "Count")
```
