...
group_data2 <- combined_data %>%
 pivot_longer(cols = Mean:IQR, names_to = "Summary_Variable" ,
              values_to = "Value")
```
...
```{r}
model <- lm(Happiness_Score ~ Economy + Social + Health + Freedom +
...
visualize(null_distribution_correlation) +
  shade_p_value(obs_stat = happiness_obs_stat, direction = "two_sided") +
    labs(title = "Null Distribution of Happiness Score",
